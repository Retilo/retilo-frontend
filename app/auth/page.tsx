...
function AuthPageInner() {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await api.post("/v1/auth/forgot-password", { email: form.email })
      setForgotSent(true)
    } catch (err: unknown) {
        ...
        >
          ...
          {mode !== "forgot" && (
            <div className="flex w-full rounded-xl p-1 mb-5 border border-gray-200 bg-gray-50">
              {(["login", "register"] as Mode[]).map((t) => (
                <button
                  key={t}
                  onClick={() => { setMode(t as Mode); setError("") }}
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {mode === "forgot" && forgotSent && (
              <p className="text-emerald-700 text-xs bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2">
            ...
            >
              {loading ? "Please wait…"
                ...
                : forgotSent ? "Email sent" : "Send reset link"}
            </button>
          </form>
