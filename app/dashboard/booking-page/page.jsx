...
function EditForm({ initialForm, isNew, locations, onSave, onBack, showToast }) {
  ...
  const set = (key) => (e) => {
    ...
    // Auto-fill slug from display name on new pages (only while slug is still empty)
    if (key === "displayName" && isNew && !form.slug) {
      const auto = val.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")
      const suffix = form.locationId ? `-loc${form.locationId}` : ""
      setForm(f => ({ ...f, displayName: val, slug: auto + suffix }))
      return
    }
    setForm(f => ({ ...f, [key]: val }))
  }
      ...
      <SectionCard icon={Globe} title="Identity">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2">
            ...
            {slugError
              ? <p className="text-xs mt-1" style={{ color: RED }}>{slugError}</p>
              : !isMaster && (
                <p className="text-xs mt-1" style={{ color: TEXT_FAINT }}>
                  Must be unique — each location needs its own slug (e.g. <span style={{ color: TEXT_MUTED }}>canteen-hyd</span>, <span style={{ color: TEXT_MUTED }}>canteen-bng</span>)
                </p>
              )
            }
          </div>
