...
function MarqueePathItem({
  ...
  useEffect(() => {
    const unsubscribe = itemOffset.on("change", (value: string) => {
      const match = value.match(OFFSET_PERCENT_RE);
      if (match?.[1]) {
...
const MarqueeAlongSvgPath = ({
  ...
  // Stable ID for SSR + hydration (Math.random would mismatch server vs client)
  ...
  return (
    <div
      ...
      ref={container}
    >
      <div className="relative" ref={marqueeContainerRef} style={{ contain: "layout style" }}>
        <svg
        ...
        >
          <path d={path} fill="none" id={id} ref={pathRef} stroke={showPath ? "currentColor" : "none"} />
        </svg>
