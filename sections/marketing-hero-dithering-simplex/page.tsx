  ...
  return (
    <div className="relative z-10 min-h-0 w-full flex-1 overflow-visible">
      <div className={cn("relative z-10 flex justify-between gap-4 text-base md:text-lg", HERO_PAD)}>
        ...
        <a
          className="hidden h-fit shrink-0 items-center justify-center rounded-full bg-[#f4f8e8]/10 px-4 py-2 font-medium text-[#f4f8e8] text-sm backdrop-blur-sm transition-colors duration-200 hover:bg-[#f4f8e8]/20 md:inline-flex md:text-base"
          href="#"
    ...
    >
      ...
      >
        <motion.div variants={itemVariants}>
          <span
            className="mb-4 inline-flex items-center gap-2 rounded-full border border-[#FD5BFF]/35 bg-[#301c2a]/55 px-3 py-1 font-medium text-[#FD5BFF]/90 text-xs tracking-wide backdrop-blur-sm"
            style={{ boxShadow: "0 0 0 1px rgba(171,250,25,0.18), 0 8px 24px rgba(0,0,0,0.24)" }}
          >
            <Grid3x3 className="h-3.5 w-3.5 text-[#FD5BFF]" />
            Simplex dither · 4x4
          </span>
        </motion.div>
        ...
        <motion.h1
          className={cn(
            "relative max-w-[min(100%,48rem)] text-balance font-bold text-[#f4f8e8] text-[3rem] leading-[1.08] tracking-[-0.035em] [text-shadow:0_2px_24px_rgba(0,0,0,0.55)] sm:text-[3.5rem] md:text-[4rem] lg:text-[4.5rem]",
            HERO_HEADLINE_PB
          )}
          variants={{
            hidden: {},
        ...
        <motion.p
          className="-mt-2 mb-6 max-w-2xl text-pretty font-medium text-[#f4f8e8] text-xl leading-relaxed [text-shadow:0_1px_20px_rgba(0,0,0,0.65),0_0_1px_rgba(0,0,0,0.4)] sm:text-2xl md:text-[1.75rem]"
          variants={itemVariants}
        >
          {DESCRIPTION}
        </motion.p>
        ...
        <motion.div className="mt-auto flex flex-col gap-3 sm:flex-row sm:gap-4" variants={itemVariants}>
          ...
          >
            <Button
              className="rounded-full bg-[#FD5BFF] px-7 font-medium text-[#301c2a] hover:bg-[#c4ff3d]"
              size="lg"
              style={{
                boxShadow:
                  "0 0 0 1px rgba(171,250,25,0.4), 0 4px 24px rgba(171,250,25,0.2), 0 4px 20px rgba(0,0,0,0.2)",
              }}
            >
              Get Started
          ...
          >
            <Button
              className="rounded-full border-2 border-[#FD5BFF]/80 bg-[#301c2a]/40 px-7 font-medium text-[#f4f8e8] shadow-[0_0_0_1px_rgba(171,250,25,0.2),0_4px_24px_rgba(0,0,0,0.35)] backdrop-blur-sm hover:border-[#FD5BFF] hover:bg-[#FD5BFF]/20 hover:text-white hover:shadow-[0_0_0_1px_rgba(171,250,25,0.35),0_6px_28px_rgba(171,250,25,0.12)]"
              size="lg"
              variant="outline"
            >
              Learn More
...
function DitheringHeroFooterPanel({ footerListVariants, footerLinkVariants }: FooterPanelProps) {
  ...
  const linkLabelVariants: Variants = {
    rest: { opacity: 0.48 },
    hover: { opacity: 1, transition: { duration: 0.2, ease: SIMPLEX_LINK_EASE } },
  };

  const linkCellVariants: Variants = {
    rest: { opacity: 0.22, y: 0 },
    hover: shouldReduceMotion
      ? { opacity: 1, y: 0 }
      : { opacity: 1, y: -1, transition: { duration: 0.2, ease: SIMPLEX_LINK_EASE } },
  };
  ...
  return (
    ...
    >
      ...
      <motion.ul
        animate="visible"
        className="relative z-10 flex w-full flex-col items-end gap-2 opacity-50 transition-opacity duration-300 ease-out hover:opacity-100 md:gap-2.5"
        initial="hidden"
        variants={footerListVariants}
      >
        {FOOTER_LINKS.map((link, i) => (
          <motion.li className="flex w-full justify-end" key={`${link.label}-${i}`} variants={footerLinkVariants}>
            <motion.a
              className={cn(
                "group inline-flex max-w-full items-baseline gap-x-3 outline-none ring-offset-2 ring-offset-white",
                "text-neutral-900 dark:text-white dark:ring-offset-neutral-950",
            ...
            >
              <span
                aria-hidden
                className="shrink-0 font-mono text-[10px] text-neutral-400 leading-none tracking-[0.22em] dark:text-neutral-500"
              >
                {String(i + 1).padStart(2, "0")}
              </span>
              <span
                aria-hidden
                className="shrink-0 select-none text-[0.65em] text-neutral-300 leading-none dark:text-neutral-600"
              >
                –
              </span>
              <motion.span
                className="min-w-0 max-w-[min(100%,18rem)] text-pretty text-right font-medium text-[0.95em] tracking-[-0.01em] md:max-w-[min(100%,22rem)] md:text-base md:tracking-tight"
                initial="rest"
              ...
              <motion.span
                aria-hidden
                className="pointer-events-none size-1 shrink-0 bg-neutral-900 dark:bg-neutral-100"
                initial="rest"
                style={{ borderRadius: 1 }}
                variants={linkCellVariants}
              />
            </motion.a>
          </motion.li>
        ))}
...
export default function DitheringSimplexHero() {
  ...
  return (
    ...
    >
      <div
        className={cn("relative flex min-h-0 flex-1 flex-col overflow-hidden bg-[#301c2a]", HERO_SHELL_RADIUS_CLASS)}
      >
